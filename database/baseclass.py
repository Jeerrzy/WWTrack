...
class Database(object):
    ...
    def append_new_file(self, new_file_path):
        ...
        if basename not in self.file_name_list:
            ...
            _dict = {
                ...
                'fps': int(fps),
                'frames': int(frames),
                'cache': os.path.join(self.cache_dir, name),
                'flag': False
            }
    ...
    def export_excel(self):
        ...
        for file_obj in self.file_obj_list:
            worksheet = workbook.add_worksheet(file_obj['name'])
            worksheet.activate()
            header_names = ['ID', 'Color', 'Distance_pixel', 'Frames_num', 'Positions_R1R2R3NA', 'Speed_pixel/frame',
                            'Speed', 'FileName', 'TotalFrames', 'FPS', 'Resolution']
            ...
            with open(os.path.join(file_obj['cache'], 'math', 'result.json'), 'r') as f:
                result = json.load(f)
            worm_nums = len(result.keys())
            for row in range(1, worm_nums+1):
            ...
            worksheet.write(1, 7, file_obj['name'], content_style)
            worksheet.write(1, 8, str(file_obj['frames']), content_style)
            worksheet.write(1, 9, str(file_obj['fps']), content_style)
            worksheet.write(1, 10, str(file_obj['width']) + ',' + str(file_obj['height']), content_style)
            for i in range(worm_nums):
                id_dict = result[str(i+1)]
                distance_pixel = np.array(id_dict['speed']).sum()
                frames_num = len(id_dict['frame'])
                positions = id_dict['position']
                pos_r1, pos_r2, pos_r3, pos_na = positions.count('r1'), positions.count('r2'), positions.count('r3'), positions.count('N/A')
                speed_pf = round(distance_pixel/frames_num, 6)
                speed_true = speed_pf * self.cfg['gain_multiple']
                worksheet.write(i + 1, 0, str(i+1), content_style)
                color_fmt = workbook.add_format({'bold': True})
                color_fmt.set_bg_color(BGR_to_Hex(colors[i % len(colors)]))
                worksheet.write(i + 1, 1, '', color_fmt)
                worksheet.write(i + 1, 2, str(distance_pixel), content_style)
                worksheet.write(i + 1, 3, str(frames_num), content_style)
                worksheet.write(i + 1, 4, str(pos_r1)+','+str(pos_r2)+','+str(pos_r3)+','+str(pos_na), content_style)
                worksheet.write(i + 1, 5, str(speed_pf), content_style)
                worksheet.write(i + 1, 6, str(speed_true), content_style)
        workbook.close()
