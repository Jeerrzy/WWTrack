...
class MainViewer(QWidget):
    ...
    def __init__(self, database, logger, parent=None):
        ...
        # self.settings_btn = IconButton(icon="QPushButton{border-image: url(./source/icon/settings.png)}", size=30)
        # self.layout.addWidget(self.settings_btn, 10, 0, 1, 1)
        ...
        self.previous_btn = IconButton(icon="QPushButton{border-image: url(./database/icon/previous.png)}", size=30)
        self.layout.addWidget(self.previous_btn, 10, 3, 1, 1)
        self.next_btn = IconButton(icon="QPushButton{border-image: url(./database/icon/next.png)}", size=30)
        self.layout.addWidget(self.next_btn, 10, 5, 1, 1)
        self.circle_btn = IconButton(icon="QPushButton{border-image: url(./database/icon/circle.png)}", size=35)
        self.layout.addWidget(self.circle_btn, 10, 7, 1, 1)
        self.multiple_btn = IconButton(icon="QPushButton{border-image: url(./database/icon/multiple.png)}", size=35)
        self.layout.addWidget(self.multiple_btn, 10, 8, 1, 1)
...
class MyGraphicItem(QGraphicsPixmapItem):
    def __init__(self, database, parent=None):
        ...
        self.file_obj = None
        self.default_pixmap_path = './database/icon/worm.png'
        self.pix_map = None
        # 缩放以及位移参数
        self.iw, self.ih = 0, 0
        self.vw, self.vh = 0, 0
    ...
    def update_pix_map(self, update_parameter=False):
        if self.file_obj is not None and self.file_obj['flag']:
            opencv_array = cv2.imread(os.path.join(self.file_obj['cache'], 'visual', 'cache0.png'))
        else:
            opencv_array = cv2.imread(self.default_pixmap_path)
        # 绘制圆心
        """"""
        radius_list = self.database.cfg['radius']
        color_list = [(255, 0, 0), (0, 255, 0), (0, 0, 255)]
        center = self.database.cfg['center']
        h, w, _ = opencv_array.shape
        for i in range(3):
            cv2.circle(opencv_array, (int(w*center[0]), int(h*center[1])), int(radius_list[i]*min(h, w)), color_list[i], 2)
        """"""
